...
const PostWriteContent = () => {
  ...
  useEffect(() => {
    ...
    const fetchDraft = async (categoryList) => {
      try {
        ...
        if (matched) setCategory(String(matched.id ?? matched.somId));
      } catch (error) {
        openModal({
          title: "불러오기 실패",
  ...
  // 3) 이미지 업로드 (Toast UI Hook)
  const handleImageUpload = async (blob, callback) => {
    if (uploadFlag.current) return false; 
  ...
  const handleSubmit = async (e) => {
    ...
    if (!title.trim()) return openModal({ title: "제목을 입력해주세요", confirmText: "확인" });
    if (!category.trim()) return openModal({ title: "카테고리를 선택해주세요", confirmText: "확인" });
    if (!content.trim()) return openModal({ title: "내용을 입력해주세요", confirmText: "확인" });
